...
MastraとNext.jsを統合したAI Agentアプリケーションを実装したいです。

このAI Agentにはステーブルコイン決済に特化したMCPである x402 MCPを統合し、自然言語のやりとりのみでステーブルコイン決済が可能となるような仕様にしたいです。

さらにこのAI AgentアプリケーションをAmazon Bedrock AgentCoreを使って簡単にデプロイできるようにしたいです。そしてそのリソースはAWS CDKを使って管理したいです。

### 段階的なSpec駆動開発

このプロジェクトは**段階的なSpec駆動開発**を重視し、アプリケーションコンポーネントとCDKコンポーネントに実装フェーズを明確に分離して開発を進めてください。
